import { arrayToGenerator, splitStringByMaxLength } from "./utils.js";
const nonhazardousLabel = (data) => {
    const description = arrayToGenerator(splitStringByMaxLength(data['waste-description'], 40));
    return `<text class="non-haz-title" x="11.911" y="3.509">NONHAZARDOUS WASTE</text>
      ...
      <text x="20" y="140">
        ...
        <tspan x="20" dy="1.1em" id="description2">${description.next().value}</tspan>
        <tspan x="20" dy="1.1em" id="description3">${description.next().value}</tspan>
        <tspan x="20" dy="1.1em" id="description4">${description.next().value}</tspan>
      </text>`;
};
